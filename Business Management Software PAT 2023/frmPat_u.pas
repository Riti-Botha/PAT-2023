...
begin
  dmPAT.conRH.ConnectionString :=
    'Provider=Microsoft.Jet.OLEDB.4.0;Data Source=RoyaleHighData.mdb;Persist Security Info=False;';
...
begin
  ...
  begin
    ...
    begin
      ...
      // toets of daar 'n duplicate value is
      for i7 := 1 to iUN do
      begin
        if suser = arrSpelerNaam[i7] then
        begin
...
begin
  ...
  begin
    sprys := redInfo.Lines[4];
    // Vat net getal waardes van prys
    for i := 1 to length(sprys) do
    begin
      if (sprys[i] in ['0' .. '9']) or (sprys[i] = '.') then
      begin
    ...
    // Probeer prys dan weer na getalle omskakel
    Try
      rprys := strtofloat(snewprice);
    Except
      on EConvertError do
    ...
    // Kry net die heelgetal waarde van rprys, sodat ons akkuraat
    // kan toets of die heelgetal gedeelte 'n priemgetal is of nie
    iNum := trunc(rprys);

    // Wys of prys 'n priemgetal is
  ...
  begin
    // Vir ingeval iemand nie die items sien nie en steeds wil kyk of die prys
    // 'n priemgetal is
...
begin
  ...
  // Gee 'n datasource waarde
  // Maak bItems = true, sodat ons maklik kan wys/edit/insert/delete waar nodig
  dbgInfo.DataSource := dmPAT.dsrItems;
  bItems := true;
  redInfo.Lines.Clear;
  with dmPAT do
...
begin
  ...
  // Check which username is being used and then check if the
  // username matches the pin
  for i := 1 to inume do
  begin
...
begin
  with dmPAT do
  begin
    // Wys die volgende waarde in tabel
    if bItems = true then
    begin
      tblItems.Next;
    ...
    // Vertoon die volgende waarde in die rich edit
    if bItems = true then // wys item waardes
    begin
      sprys := currtostr(tblItems['Prys']); // verander die waardes na strings
      rprys := strtofloat(sprys); // -----------------> om te vertoon
      sprys := floattostrf(rprys, ffcurrency, 6, 2);
      redInfo.Lines.Add('Item: ' + #13 + tblItems['Item'] + #13);
      redInfo.Lines.Add('Price: ' + #13 + sprys + #13);
      redInfo.Lines.Add('Season: ' + #13 + datetostr(tblItems['ItemSeisoen']));
...
begin
  with dmPAT do
  begin
    // Wys die vorige waarde in tabel
    if bItems = true then
    begin
      tblItems.prior;
    ...
    redInfo.Lines.Clear;
    // Vertoon die vorige waarde in die rich edit
    if bItems = true then // wys item waardes
    begin
      sprys := currtostr(tblItems['Prys']); // verander die waardes na strings
      rprys := strtofloat(sprys); // -----------------> om te vertoon
...
VAR
  ...
  rprys: real;
begin
  // Gee weer 'n source waarde om die ranglys die keer te vertoon
  dbgInfo.DataSource := dmPAT.dsrRanglys;
  bItems := false;
  redInfo.Lines.Clear;
  with dmPAT do
  begin // Kode vanaf voorheen in PAT gekopie en geplak
    // vertoon alle item waardes in die richedit as bitems steeds waar is
    if bItems = true then
    begin
      sprys := currtostr(tblItems['Prys']);
      rprys := strtofloat(sprys);
    ...
    begin // vertoon alle rang waardes in rich edit
      redInfo.Lines.Add('Level: ' + #13 + inttostr(tblRanglys['Rang']) + #13);
      redInfo.Lines.Add('Username: ' + #13 + (tblRanglys['SpelerNaam']) + #13);
      redInfo.Lines.Add('Most Expensive Item: ' + #13 + tblRanglys
        ['DuursteItem']);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          // vertoon boodksap dat rekord gevind is

          tblItems.Delete;
          showmessage('Record found and deleted.');
          exit;
        end
        else
        begin
          // lees deur tabel tot dat waarde gekry is
          tblItems.Next;
        end;
...
begin
  // gebruik on change sodat gebeure maklik vloei
  with dmPAT do
  begin
